...
func (q *SessionRepository) CreateSession(ctx context.Context, userID int32, tokenHash string, absoluteExpiry time.Time) (Session, error) {
	s := Session{
		TokenHash:         tokenHash,
		UserID:            userID,
...
func (q *SessionRepository) GetSessionUser(ctx context.Context, tokenHash string) (User, error) {
	var s Session
...
func (q *SessionRepository) DeleteSessionByTokenHash(ctx context.Context, tokenHash string) error {
...
func (q *SessionRepository) DeleteSessionsForUser(ctx context.Context, userID int32) error {
	return q.db.WithContext(ctx).Where("user_id = ?", userID).Delete(&Session{}).Error
}

func (q *SessionRepository) DeleteExpiredSessions(ctx context.Context) error {
