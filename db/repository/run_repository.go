...
func (q *RunRepository) ListAllRuns(ctx context.Context) ([]Run, error) {
	var runs []Run
...
func (q *RunRepository) UpdateRunLog(ctx context.Context, id int32, content string, status string) error {
	...
	return NewRunEventRepository(q.db).EnqueueRunEvent(ctx, RunEvent{TaskID: eventTaskID, RunID: r.ID, EventType: RunEventTypeLifecycleStatus, Payload: status, DedupeKey: fmt.Sprintf("run:%d:status:%s", r.ID, status)})
}
...
func (q *RunRepository) MarkRunStale(ctx context.Context, runID int32, reason string) (bool, error) {
	var run Run
...
func (q *RunRepository) SetRunRootID(ctx context.Context, id int32, rootID int32) error {
...
// delegation sessions.
func (q *RunRepository) ListChildRuns(ctx context.Context, parentRunID int32) ([]Run, error) {
	var runs []Run
...
func (q *RunRepository) ListDescendantRuns(ctx context.Context, rootRunID int32) ([]Run, error) {
	var runs []Run
	err := q.db.WithContext(ctx).
		Preload("Task").Preload("Agent").
...
func (q *RunRepository) UpdateRunSession(ctx context.Context, id int32, sessionID string) error {
...
func (q *RunRepository) GetRunBySessionID(ctx context.Context, sessionID string) (Run, error) {
	var r Run
	err := q.db.WithContext(ctx).Where("session_id = ?", sessionID).First(&r).Error
	return r, err
}

func (q *RunRepository) GetRunningRunsByTaskID(ctx context.Context, taskID int32) ([]Run, error) {
	var runs []Run
	err := q.db.WithContext(ctx).Where("task_id = ? AND status = ?", taskID, "running").Find(&runs).Error
	return runs, err
}

func (q *RunRepository) GetOrchestratorRun(ctx context.Context, taskID int32) (Run, error) {
	var task Task
...
func (q *RunRepository) ListOrchestratorSessions(ctx context.Context, orchestratorRunID int32) ([]Run, error) {
	var runs []Run
	err := q.db.WithContext(ctx).
		Preload("Agent").
...
func (q *RunRepository) ListOrchestratorsByTask(ctx context.Context, taskID int32) ([]Run, error) {
	var runs []Run
	err := q.db.WithContext(ctx).
...
func (q *RunRepository) ListWaitingOrchestrators(ctx context.Context) ([]Run, error) {
	var runs []Run
	err := q.db.WithContext(ctx).
...
func (q *RunRepository) IncrementRunRecoveryAttempts(ctx context.Context, runID int32) error {
	var run Run
	if err := q.db.WithContext(ctx).First(&run, runID).Error; err != nil {
		return err
	}
...
func (q *RunRepository) GetRunningRuns(ctx context.Context) ([]Run, error) {
	var runs []Run
...
func (q *RunRepository) IsRunStale(ctx context.Context, runID int32, threshold time.Duration) (bool, error) {
	cutoff := time.Now().Add(-threshold)
	var count int64
	err := q.db.WithContext(ctx).
		Model(&Run{}).
...
func (q *RunRepository) GetRunTokenStats(ctx context.Context, runID int32) (RunTokenStats, error) {
	var r Run
...
func (q *RunRepository) ListCompletedRunsByTask(ctx context.Context, taskID int32) ([]Run, error) {
	var runs []Run
	err := q.db.WithContext(ctx).
...
func (q *RunRepository) GetLatestRunByTask(ctx context.Context, taskID int32) (Run, error) {
	var r Run
	err := q.db.WithContext(ctx).
...
func (q *RunRepository) UpdateRunName(ctx context.Context, id int32, name string) error {
...
func (q *RunRepository) CountRootRunsThrough(ctx context.Context, taskID, rootRunID int32) (int64, error) {
	var count int64
...
func (q *RunRepository) CountSubsessionRunsThrough(ctx context.Context, rootRunID, runID, agentID int32) (int64, error) {
	var root Run
...
func (q *RunRepository) UpdateRunRecoveryMetadata(ctx context.Context, runID int32, reason, initiator, target string) error {
	var run Run
	if err := q.db.WithContext(ctx).First(&run, runID).Error; err != nil {
		return err
	}
...
func (q *RunRepository) RecordResumeError(ctx context.Context, runID int32, resumeErr, recoverableStatus string) error {
...
func (q *RunRepository) MarkRunRecoverable(ctx context.Context, runID int32, status string, sequence int64, reason string) error {
