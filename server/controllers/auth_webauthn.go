...
func (api *API) RegisterFinish(w http.ResponseWriter, r *http.Request) {
	...
	api.respondJSON(w, http.StatusCreated, map[string]any{"user": userResponse{ID: user.ID, Email: user.Email, IsAdmin: api.isInstanceAdmin(r.Context(), user.ID)}, "unlocked": true})
}
...
func (api *API) finishAssertion(w http.ResponseWriter, r *http.Request, purpose string, issueSession bool) {
	...
	api.respondJSON(w, http.StatusOK, map[string]any{"user": userResponse{ID: user.ID, Email: user.Email, IsAdmin: api.isInstanceAdmin(r.Context(), user.ID)}, "unlocked": unlocked})
}
