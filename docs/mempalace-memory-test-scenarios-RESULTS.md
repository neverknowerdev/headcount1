...
**Revised verdict:** there are **two distinct, real findings** here, not one:
1. An **intermittent write-to-search indexing race** for cross-agent recall of very-recently-written facts — not deterministic, not role-specific, correlated with real elapsed time between write and read. 3 of 4 attempts across this scenario failed; the successful attempt happened to have more wall-clock time between write and read.
2. A **ranking precision problem**, independent of #1: even when content *is* indexed, low-value `technical`-room transcript noise can outrank the actually relevant fact for realistic queries, unless the caller explicitly narrows to `room: 'general'`/`'decisions'`. This is fixable today, by callers, with the existing `room` parameter — worth considering as a default recommendation for `recall_memory` callers asking status-style questions, or as a ranking-weight adjustment (down-weighting `technical`-room hits relative to `general`/`decisions`) if this generalizes beyond this one scenario.
...
| 7. Cross-role blocker | Yes | **Failed 3 of 4 runs; passed on the 4th** — intermittent, not deterministic | Two separate real issues: (1) an intermittent write-to-search indexing race for cross-agent recall, correlated with wall-clock timing, not role-specific; (2) a genuine ranking problem where `technical`-room transcript noise outranks real content — `room` filtering fixes (2) but can't fix (1) |
...
1. Confirm the indexing-race root cause with real instrumentation: poll `/api/memory/status` or the embedding backend's own metrics between the CTO write and the cross-agent read across many repeated runs, to measure the actual distribution of the lag rather than inferring it from wall-clock proxies (test setup speed) as this pass did.
2. Consider whether `recall_memory` should down-weight or exclude the `technical` room (transcript-mined content) by default for status-style queries, or ship a documented recommendation that callers pass `room` when asking about current state — the room-filter experiment showed this is a real, fixable precision problem today.
3. Decide, as a design question (not urgent): should `recall_memory` actually consult the KG for current-truth preference, matching the design docs, or should the docs be corrected to describe the shipped (pure-search) behavior? Right now the two disagree.
4. Run scenario 7 several more times (5-10x) to get an actual failure rate for the indexing race, rather than the small, noisy 3-of-4 sample this pass produced.
