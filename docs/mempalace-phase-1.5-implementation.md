...
1. Lower `check_duplicate` threshold to **0.83** (tune later; make it a package const `rememberDupThreshold`).
   > **Implementation deviation (measured, revised twice):**
   > - First pass: dedup-checked the *stored* form (content prefixed `"[closet] [kind] …"`, matching what old drawers already carried), threshold 0.72. Measured with the shipped MiniLM embeddings: same-task paraphrases ≈0.83, cross-task paraphrases ≈0.75, different facts with shared vocabulary ≈0.54.
   > - Follow-up measurement found the prefix itself was the problem, not just the threshold: two agents filing the *same* fact under different `kind` tags ("learning" vs "note") dragged a genuine duplicate from 0.83 down to 0.75 raw — and a real decision reversal on the same topic ("use WebSockets" → "use SSE, WebSockets rejected") scored 0.75, i.e. *above* the 0.72 cut. At 0.72 that reversal would have been silently swallowed as "already known."
   > - **Shipped:** `remember` now stores and dedup-checks RAW content — no `[closet] [kind]` prefix at all. The task association isn't lost (it still rides on `source_file`, which `mempalace_search` echoes back as `source_path`); only the `note`/`learning` distinction is no longer separately recoverable from stored text (room still distinguishes `decision` from the rest). Raw-content similarity: paraphrases 0.856–0.944, the decision-reversal case 0.754, different facts sharing vocabulary 0.040–0.399. `rememberDupThreshold = 0.80` sits with ~0.05 margin on both sides of the (0.754, 0.856) safe window. Pinned by the T3 paraphrase-gauntlet e2e test (`e2e/tests/memory_dedup_gauntlet.spec.ts`).
   > - Artifact ingestion (W1) intentionally keeps its own `"[closet] [artifact <filename>] <description>"` prefix — the filename/description there is real, non-redundant signal, not boilerplate, and artifact-dedup collisions are a much rarer scenario than `remember`'s.
