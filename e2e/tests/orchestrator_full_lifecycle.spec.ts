    ...
    test('coordinates CTO research, architecture routing, QA repair, stop, fork, and re-verification', async ({ request }) => {
        ...
        await postJSON(request, `${env.E2E_MOCK_PROVIDER_URL}/__test/set-scenario`, {
            model: 'e2e-orchestrator-model', entries: [
                { tool_call: { id: 'launch-ceo', name: 'run_new_session', arguments: { agent_name: ceo.name, title: 'Confirm product direction', prompt: 'Confirm the product direction with the human, then hand the decision to the CTO.' } } },
                { tool_call: { id: 'inspect-ceo', name: 'get_session', arguments: { session_id: 0 } } },
                { tool_call: { id: 'launch-cto', name: 'run_new_session', arguments: { agent_name: cto.name, title: 'Design technical solution', prompt: 'Design the technical solution, coordinate research workers, write the technical specification artifact, and finish the design handoff.' } } },
                ...
                { tool_call: { id: 'launch-coder', name: 'run_new_session', arguments: { agent_name: coder.name, title: 'Implement technical solution', prompt: 'Implement the CTO technical specification and ask the task owner about architecture if needed.' } } },
                ...
                { tool_call: { id: 'launch-qa', name: 'run_new_session', arguments: { agent_name: qa.name, title: 'Run first QA pass', prompt: 'Test the implementation independently and report every issue to the task owner.' } } },
