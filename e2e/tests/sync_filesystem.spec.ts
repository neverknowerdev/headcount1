...
test.describe.serial('Entity filesystem sync (export / import)', () => {
    ...
    const compDataDir = path.join(headcountBase, 'data', shortName);
    const compSettingsDir = path.join(headcountBase, 'companies', shortName);
    ...
    test('writes entity files to filesystem on create and update', async ({ request }) => {
        ...
        const providerPath = path.join(headcountBase, 'data', 'llm-providers', `${providerId}.json`);
...
test.describe('Filesystem as Source of Truth', () => {
    ...
    const headcountBase = path.join(env.E2E_HEADCOUNT_HOME, '.headcount1');
    const companyPath = path.join(headcountBase, 'data', companyShortName);
    const backupPath = path.join(os.tmpdir(), `headcount-backup-${Date.now()}`);
    ...
    test('should maintain filesystem as source of truth and sync back', async ({ page }) => {
        ...
        const projectPath = path.join(headcountBase, 'data', 'artifacts', companyShortName, 'Test Project');
