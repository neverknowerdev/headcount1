...
export function loadE2EEnv(): E2EEnv {
    ...
    if (!data.E2E_MOCK_PROVIDER_URL || !data.E2E_TEST_REPO_URL || !data.E2E_HEADCOUNT_HOME) {
        throw new Error(
            `loadE2EEnv: env file at ${envFile} is missing E2E_MOCK_PROVIDER_URL, E2E_TEST_REPO_URL, or E2E_HEADCOUNT_HOME. ` +
            `Got: ${JSON.stringify(data)}`,
        );
    }
    cached = { E2E_MOCK_PROVIDER_URL: data.E2E_MOCK_PROVIDER_URL, E2E_TEST_REPO_URL: data.E2E_TEST_REPO_URL, E2E_HEADCOUNT_HOME: data.E2E_HEADCOUNT_HOME };
    return cached;
}
