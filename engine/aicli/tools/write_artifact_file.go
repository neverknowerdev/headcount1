...
type WriteArtifactFile struct {
	...
	onWrite func(ctx context.Context, filename, content string) (string, error)
}

func NewWriteArtifactFile(onWrite func(ctx context.Context, filename, content string) (string, error)) *WriteArtifactFile {
...
func (t *WriteArtifactFile) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	msg, err := t.onWrite(ctx, p.Filename, p.Content)
	if err != nil {
