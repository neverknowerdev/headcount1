...
func (b *BrowserUse) click(ctx context.Context, selector string) (string, error) {
	...
	if err := chromedp.Run(bc, chromedp.Click(selector, chromedp.NodeVisible)); err != nil {
...
func (b *BrowserUse) typeText(ctx context.Context, selector, text string) (string, error) {
	...
	if err := chromedp.Run(bc, chromedp.SendKeys(selector, text, chromedp.NodeVisible)); err != nil {
...
func (b *BrowserUse) getText(ctx context.Context, selector string) (string, error) {
	...
	var text string
	if err := chromedp.Run(bc, chromedp.Text(selector, &text)); err != nil {
...
func (b *BrowserUse) getHTML(ctx context.Context, selector string) (string, error) {
	...
	var html string
	if err := chromedp.Run(bc, chromedp.OuterHTML(selector, &html)); err != nil {
...
func (b *BrowserUse) screenshot(ctx context.Context) (string, error) {
	...
	var buf []byte
	if err := chromedp.Run(bc, chromedp.CaptureScreenshot(&buf)); err != nil {
...
func (b *BrowserUse) executeJS(ctx context.Context, script string) (string, error) {
	...
	var result any
	if err := chromedp.Run(bc, chromedp.Evaluate(script, &result)); err != nil {
