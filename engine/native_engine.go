...
func (e *NativeEngine) executeSession(ctx context.Context, task db.Task, mode string, parent *parentSession, resumeRun *db.Run) string {
	...
	if gitProject && gitMgr != nil && status == runStatusCompleted {
		if author, authorErr := e.commitAuthorForTask(ctx, task); authorErr != nil {
			...
			e.tryGitCommit(ctx, proxyLogger, gitMgr, workspacePath, task, agent, author)
		}
...
func (e *NativeEngine) tryGitCommit(ctx context.Context, logger *logging.ProxyLogger, gitMgr *git.GitManager, workspacePath string, task db.Task, agent db.Agent, author git.CommitAuthor) bool {
