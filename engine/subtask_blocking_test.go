...
// TestCreateSubtaskBlocksAndReturnsResult verifies the delegation flow:
// create_subtask waits for the subtask run to finish and its tool result
// (visible in the parent's next LLM request) carries the subtask's status,
// run ID, and result summary.
func TestCreateSubtaskBlocksAndReturnsResult(t *testing.T) {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case n == 2:
			// Subtask turn 1: finish immediately with a result.
		...
		default:
			// Parent turn 2 (sees the subtask result): finish the parent.
			resp = toolCallResponse("finish_task",
	...
	mu.Lock()
	defer mu.Unlock()
	require.GreaterOrEqual(t, len(bodies), 3, "expected parent turn after subtask finished")
	last := bodies[len(bodies)-1]
	assert.Contains(t, last, "finished with task status", "subtask result summary missing")
	assert.Contains(t, last, "Research complete: X is feasible.", "subtask result_description missing")
	assert.Contains(t, last, "expand_run_result", "run handoff pointer missing")

	// The subtask itself ended in-review and stored the detailed handoff.
...
// paths the sandbox forbids fail fast instead of wasting a whole run.
