        ...
        {repoSuccess && (
          <div className="p-3 bg-green-50 border border-green-200 rounded text-green-700 text-sm">{repoSuccess}</div>
			...
			<div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
				<div className="flex items-start justify-between gap-3"><div className="flex gap-2"><GitBranch className="mt-0.5" size={19}/><div><h3 className="font-medium text-gray-900">GitHub repository</h3><p className="mt-1 text-xs text-gray-600">Recommended. Select a repository available through one of your connected GitHub MCP accounts.</p></div></div><a href={`/companies/${shortName}/mcp-servers`} className="inline-flex shrink-0 items-center gap-1 text-xs font-medium text-indigo-700 hover:underline">Manage GitHub accounts <ExternalLink size={12}/></a></div>
				{githubStatus?.configured && githubRepos.length > 0 ? <><input value={repoSearch} onChange={e => setRepoSearch(e.target.value)} placeholder="Search repositories…" className="mt-3 w-full rounded border p-2 text-sm"/><select value={selectedGithubRepo?.id || ''} onChange={e => { const repo = githubRepos.find(x => String(x.id) === e.target.value) || null; setSelectedGithubRepo(repo); if (repo) setRepositoryUrl(repo.clone_url); }} className="mt-2 w-full rounded border p-2 text-sm"><option value="">Select a connected repository</option>{githubRepos.filter(r => r.full_name.toLowerCase().includes(repoSearch.toLowerCase())).map(r => <option key={r.id} value={r.id}>{r.full_name}</option>)}</select></> : <p className="mt-3 text-sm text-gray-600">{githubStatus?.configured ? 'No permitted repositories found. Add or reconnect a GitHub account in MCP Servers, then choose its repository access in GitHub.' : 'GitHub integration is unavailable on this environment. Ask an administrator to configure the GitHub App.'}</p>}
			</div>
