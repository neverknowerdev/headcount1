                ...
                {step === 2 && (
                    <form className="mt-8 space-y-6" onSubmit={handleCreateProvider}>
                        <div className="rounded-md shadow-sm -space-y-px flex flex-col gap-4">
                            ...
                            <button type="button" onClick={handleTestProvider} disabled={isTesting} className="w-full bg-gray-100 text-gray-800 py-2 px-4 rounded-md border font-medium">
                                {isTesting ? 'Testing...' : 'Test Connection'}
                            </button>

                            {testResult === 'success' && <p className="text-green-600 text-sm">Connection successful!</p>}
                            {testResult === 'error' && <p className="text-red-600 text-sm">Connection failed. Check details and try again.</p>}
                        </div>
